...
CollisionDetector.prototype.isGrounded = function(object) {
    ...
    var logicalY = 12 - Math.floor(y.pos / 50) + 1;
    if (y.speed == 0
...
CollisionDetector.prototype.mapCollisions = function(object) {
    var x = object.vector.x;
    var y = object.vector.y;
    ...
    if (x.pos >= this.map[0].length*this.TILE_WIDTH-100) {
        ...
        if (Math.sign(x.speed) == 1 && this.map[logicalY][Math.floor(x.pos / this.TILE_WIDTH + 2)] == 1) {
            x.pos = Math.floor(x.pos / this.TILE_WIDTH) * this.TILE_WIDTH;
            x.speed = 0;
        }
        if (Math.sign(x.speed) == -1 && this.map[logicalY][Math.floor(x.pos / this.TILE_WIDTH)] == 1) {
